...
import { editKirja, getKirja } from "./kirjat";
...
function KirjalomakeEdit() {
  ...
  const haeKirja = async () => {
    try {
      const response = await getKirja(id);
      if (response.status === 200) {
        setKirja(response.data);
      } else {
        navigate("/virhe", {
          state: { viesti: "Kirjan hakeminen ei onnistunut." },
        });
      }
    } catch {
      navigate("/virhe", {
  ...
  const muutaKirja = async (e) => {
    ...
    try {
      const response = await editKirja(id, formData);
      if (response.status === 200) {
        navigate("/listaa", {
          state: {
            message: "Kirja muokattu onnistuneesti!",
            severity: "success",
          },
        });
      }
      ...
      setSnackbarMessage(
        "Muokkaaminen ei onnistunut. Yritä myöhemmin uudelleen"
      );
  ...
  let kuvaNimi = "";
  if (kirja.kuva instanceof File) {
    kuvaNimi = kirja.kuva.name;
  } else if (typeof kirja.kuva === "string" && kirja.kuva.trim() !== "") {
    kuvaNimi = kirja.kuva;
  }
  ...
  return (
    <Container sx={{ py: 4 }} maxWidth="md">
      ...
      <Paper sx={{ p: 4 }}>
        <Box component="form" onSubmit={muutaKirja}>
          <Stack spacing={{ xs: 3 }} direction="column" useFlexGap>
            ...
            <TextField
              ...
              onChange={(e) => muutaLukuajankohta(e.target.value)}
            ...
            <FormControl>
              ...
              <RadioGroup value={kirja.laji} name="laji" onChange={muutaArvo}>
                <FormControlLabel
                  value="romaani"
                  control={<Radio />}
                  label="Romaani"
